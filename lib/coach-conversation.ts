...
export function coachConversationContinuation(
  ...
  const affirmative = /^(?:oui|ok|ok\s+vas-?y|d['’]?accord|vas-y|vasy|fais|fais-le|fait le|on\s+(?:fait|fais)\s+(?:ça|ca)|allons-y|c['’]?est parti|prépare(?:-la)?|prepare(?:-la)?)(?:\s+(?:alors|stp|s['’]?il te plaît|s['’]?il vous plaît))?$/.test(normalized) || /^(?:ok|oui)\s+(?:fais(?:-le)?|fait|vas-?y)(?:\s+(?:ça|ca|alors|stp))*$/.test(normalized);
